...
### 切换原生视图（快捷键 Alt+W / Alt+O）

支持随时按下快捷键 `Alt + W` 或 `Alt + O` 在企业微信 IM 视图与原站风格间快速切换（Linux DO 与 V2EX 均支持双向切换），设置菜单中亦提供切换选项，选择会自动记住。
...
- 划词自动解码 Base64：在页面或聊天消息中选中 Base64 字符串时自动浮现解码卡片（支持 UTF-8 中文、URL-safe Base64、无 padding、常见前缀与引号），提供一键复制与快捷打开链接/磁力链，并在外观设置中支持独立开关。
- 详情头像隐藏与排版：支持在外观设置及聊天顶栏快速隐藏/显示对话详情头像，图片自动排版支持自定义尺寸与 1:1、4:3、16:9 比例。
- V2EX 编辑器图片上传：在 V2EX 平台下，编辑器点击图片按钮、粘贴截图或拖拽图片时，自动调用 Imgur API 上传并将图片直链插入回复框中。
